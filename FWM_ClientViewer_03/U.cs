...
namespace FWM_ClientViewer_03
{
    public static class U
    {
        ...
        */
        ...
        public static object GetSetting2(string key)
        {
            const string location = CLASSNAME + ".GetSetting2(s)";
            object retVal = null;
            try
            {
                if (key == null || key.Length == 0)
                ...
                if (DataMgr.appSettings.ContainsKey("Settings"))
                {
                    try
                    {
                        jarr = (JArray)DataMgr.appSettings["Settings"];
                    }
                    catch (Exception exConv) { }
                }
                if (jarr == null)
                {
                    L.err(location, "Failed to locate settings in memory.");
                    return retVal;
                }
                for (int i = 0; i < jarr.Count; i++)
                {
                    JObject obj = (JObject)jarr[i];
                    if (obj.ContainsKey("Key") && key == (string)obj["Key"])
                    {
                        if (obj.ContainsKey("Value"))
                        {
                            retVal = obj["Value"];// no reason null shouldn't be valid here
                        }
                        break;
                    }
                }
        ...
        public static bool GetSetting2(string key, bool defaultVal)
        {
            const string location = CLASSNAME + ".GetSetting2(s,b)";
            bool retVal = defaultVal;
            try
            {
                object val = U.GetSetting2(key);
                if (val == null)
                {
                    return retVal;
                }
        ...
        public static int GetSetting2(string key, int defaultVal)
        {
            const string location = CLASSNAME + ".GetSetting2(s,i)";
            int retVal = defaultVal;
            try
            {
                object val = U.GetSetting2(key);
        ...
        public static long GetSetting2(string key, long defaultVal)
        {
            const string location = CLASSNAME + ".GetSetting2(s,l)";
            long retVal = defaultVal;
            try
            {
                object val = U.GetSetting2(key);
        ...
        public static string GetSetting2(string key, string defaultVal)
        {
            const string location = CLASSNAME + ".GetSetting2";
            string retVal = defaultVal == null ? "" : defaultVal;
            try
            {
                object val = U.GetSetting2(key);
        ...
        public static bool SetSetting2(JObject row)
        {
            const string location = CLASSNAME + ".SetSetting2(obj)";
            bool retVal = false;
            try
            {
                if (row == null)
                {
                    L.err(location, "Input setting was null.");
                    return retVal;
                }
                if (!row.ContainsKey("Key") || row["Key"] == null)
                {
                    L.err(location, "Setting was missing a key.");
                    return retVal;
                }
                if (!row.ContainsKey("Value") || row["Value"] == null)
                {
                    L.err(location, "Setting was missing a value.");
                    return retVal;
                }
                string dataType = "";
                if (!row.ContainsKey("DataType") || row["DataType"] == null)
                {
                    L.err(location, "Setting was missing a datatype, setting to (text).");
                    dataType = "text";
                }
                else 
                {
                ...
                for (int i = 0; i < jarr.Count; i++)
                {
                    ...
                    if (obj.ContainsKey("Key") && key == (string)obj["Key"])
                    {
                        if (obj.ContainsKey("Value"))
                        {
                            ...
                            {
                                case "bool": obj["Value"] = Convert.ToBoolean(row["Value"]); break;
                                case "int": obj["Value"] = Convert.ToInt32(row["Value"]); break;
                                case "long": obj["Value"] = Convert.ToInt64(row["Value"]); break;
                                case "string": goto case "text";
                                case "text": obj["Value"] = Convert.ToString(row["Value"]); break;
                                default: break;
                            }
                        ...
                        else
                        {
                            switch (dataType.ToLower())
                            {
                                case "bool": obj.Add("Value", Convert.ToBoolean(row["Value"])); break;
                                case "int": obj.Add("Value", Convert.ToInt32(row["Value"])); break;
                                case "long": obj.Add("Value", Convert.ToInt64(row["Value"])); break;
                                case "string": goto case "text";
                                case "text": obj.Add("Value", Convert.ToString(row["Value"])); break;
                                default: break;
                            }
                        }

                        // TODO - Confirm comparison works reliably
                        retVal = Convert.ToString(row["Value"]) == Convert.ToString(((JObject)jarr[i])["Value"]);
                        break;
                    }
                }
            }
        ...
        public static bool SetSetting2(string key, object value, string dataType)
        {
            const string location = CLASSNAME + ".SetSetting2(s,o)";
            bool retVal = false;
            try
            {
                if (key == null || key.Length == 0)
                ...
                for (int i = 0; i < jarr.Count; i++)
                {
                    ...
                    if (obj.ContainsKey("Key") && key == (string)obj["Key"])
                    {
                        if (obj.ContainsKey("Value"))
                        {
                            switch (dataType)
                            {
                                case "bool":    obj["Value"] = Convert.ToBoolean(value); break;
                                case "int":     obj["Value"] = Convert.ToInt32(value); break;
                                case "long":    obj["Value"] = Convert.ToInt64(value); break;
                                case "string":  goto case "text";
                                case "text":    obj["Value"] = Convert.ToString(value); break;
                                default: break;
                            }
                        ...
                        else
                        {
                            switch (dataType)
                            {
                                case "bool":    obj.Add("Value", Convert.ToBoolean(value)); break;
                                case "int":     obj.Add("Value", Convert.ToInt32(value)); break;
                                case "long":    obj.Add("Value", Convert.ToInt64(value)); break;
                                case "string":  goto case "text";
                                case "text":    obj.Add("Value", Convert.ToString(value)); break;
                                default: break;
                            }
                        }
                        retVal = Convert.ToString(value) == (string)((JObject)jarr[i])["Value"];
                        break;
                    }
                }
            }
        ...
        public static bool SetSetting2(string key, bool value)
        {
            const string location = CLASSNAME + "SetSetting2(s,b)";
            bool retVal = false;
            try
            {
                retVal = U.SetSetting2(key, value, "bool");
            }
            catch (Exception ex)
            {
                L.ex(location, ex);
        ...
        public static bool SetSetting2(string key, int value)
        {
            const string location = CLASSNAME + ".SetSetting2(s,i)";
            bool retVal = false;
            try
            {
                retVal = U.SetSetting2(key, value, "int");
            }
            catch (Exception ex)
            {
                L.ex(location, ex);
        ...
        public static bool SetSetting2(string key, long value)
        {
            const string location = CLASSNAME + ".SetSetting2(s,l)";
            bool retVal = false;
            try
            {
                retVal = U.SetSetting2(key, value, "long");
            }
            catch (Exception ex)
            {
                L.ex(location, ex);
        ...
        public static bool SetSetting2(string key, string value)
        {
            const string location = CLASSNAME + ".SetSetting2(s,s)";
            bool retVal = false;
            try
            {
                retVal = U.SetSetting2(key, value, "text");
            }
            catch (Exception ex)
            {
                L.ex(location, ex);
