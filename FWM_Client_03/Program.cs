...
namespace FWM_Client_03
{
    class Program
    {
        ...
        const string APPNAME = "FWM_Client_03";
        ...
        static double elapsedDataAging = -1;
        ...
        private static bool AlreadyRunning()
        {
            ...
            try
            {
                ...
                bool createdNew;
                mutex = new Mutex(true, "Global\\" + APPNAME + "_" + U.GetSetting2("InstanceName", ""), out createdNew);
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (!L.logInit(U.GetSetting2("logPathClient", ""), U.GetSetting2("EnableFileLogging", true)))
                {
                    L.err(location, "Failed to init file logging.");
                }
                L.l(location, "App Start - " + APPNAME + "...");

                L.l(location, "Days To Keep Data (" + U.GetSetting2("DaysToKeepData", 0) + ").");

                DateTime dtApplicationStart = DateTime.Now;

                if (!U.GetSetting2("AllowMultiInstance", false))
                {
                    if (AlreadyRunning())
                    {
                ...
                c.maxToProcess = U.GetSetting2("MaxToProcess", 50000);
                c.debug = U.GetSetting2("DebugMode", false);
                ...
                else
                {
                    ...
                    if (U.GetSetting2("EnableAutomaticReport", true))
                    {
                ...
                DateTime dtDataAgeStart = DateTime.Now;
                if (!ageData())
                {
                    // Do nothing, logs elsewhere
                }
                elapsedDataAging = (DateTime.Now - dtDataAgeStart).TotalMilliseconds;



                elapsedApplication = (DateTime.Now - dtApplicationStart).TotalMilliseconds;
                L.l(location, "Elapsed Times :: " +
                    "Load Data (" + (elapsedLoadAllData < 0 ? 0 : elapsedLoadAllData) + "), " +
                    "Read Events (" + (elapsedReadEvents < 0 ? 0 : elapsedReadEvents) + "), " +
                    "Save 1 (" + (elapsedSaveAllData1 < 0 ? 0 : elapsedSaveAllData1) + "), " +
                    "Select Qualifying (" + (elapsedSelectQualifying < 0 ? 0 : elapsedSelectQualifying) + "), " +
                    "Update Counts (" + (elapsedUpdateCounts < 0 ? 0 : elapsedUpdateCounts) + "), " +
                    "FW Add (" + (elapsedFWAdd < 0 ? 0 : elapsedFWAdd) + "), " + 
                    "FW Expire (" + (elapsedFWExpire < 0 ? 0 : elapsedFWExpire) + "), " +
                    "Save 2 (" + (elapsedSaveAllData2 < 0 ? 0 : elapsedSaveAllData2) + "), " +
                    "Publish Summary (" + (elapsedPublishSummary < 0 ? 0 : elapsedPublishSummary) + "), " + 
                    "Remove Old Data (" + (elapsedDataAging < 0 ? 0 : elapsedDataAging) + "), " +
                    "Application (" + (elapsedApplication < 0 ? 0 : elapsedApplication) + ")."
                );


                // Keep console open if not running in headless mode
                if (!U.GetSetting2("isHeadless", false))
                {
        ...
        private static bool appInit()
        {
            ...
            try
            {
                ...
                long tempAppId = U.GetSetting2("appId", 0L);
                string tempAppGuid = U.GetSetting2("appGuid", "");
        ...
        private static bool addApprovedIps()
        {
            ...
            try
            {
                string delimitedIps = U.GetSetting2("ApprovedIps", "");
        ...
        private static bool ageData()
        {
            const string location = CLASSNAME + ".ageData";
            bool retVal = false;
            try
            {
                int daysToKeepData = U.GetSetting2("DaysToKeepData", 30);
                if (daysToKeepData == 0) 
                {
                    // Zero means keep max, we age by volume only (later)
                    return retVal;
                }
                DateTime dtAge = (DateTime.Now).AddDays(-1 * daysToKeepData);

                AgeData dataAge = new AgeData();
                retVal = dataAge.removeOlderThan(dtAge);
                if (!retVal)
                {
                    L.err(location, "Failed to remove data older than (" + dtAge.ToString(TAG.DTF) + ").");
                }
            }
            catch (Exception ex)
            {
        ...
        private static bool manageFirewall()
        {
            ...
            try
            {
                bool isManageFW = U.GetSetting2("IsManageFW", false);
                ...
                int fwMinutesToReview = U.GetSetting2("FWMinutesToReview", 10080);
                ...
                if (ipBlocks.Count == 0)
                {
                    return retVal; // not an error, done
                }

                DateTime dtFWStart = DateTime.Now;

                // Wait upto X-seconds for a file-lock 
                ...
                // TODO - Sleep does not belong. Read may be coming before write, even after lock achieved.
                //L.l(location, "Waiting three seconds before reading.");
                Thread.Sleep(100);

                // Add rules
                DateTime dtStartFWAdd = DateTime.Now;
                if (!addFirewallRules(U.GetSetting2("MinFailuresToBlock", 100), ipBlocks))//min failures before blocking
                {
        ...
        private static List<IpBlock> findFailedLogins()
        {
            ...
            try
            {
                string folderEventPath = U.GetSetting2("EventFolder", "");
                ...
                // Count saved IPs for summary
                int cntTtlIps = 0;
                for (int i = 0; i < foundBlocks.Count; i++)
                {
                    if (foundBlocks[i] == null) continue;
                    if (foundBlocks[i].IpEvents == null) continue;
                    cntTtlIps += foundBlocks[i].IpEvents.Count;
                }
                summary.CntIps = cntTtlIps;
        ...
        private static List<FailedLoginEvent> readEventLog(string filePath, DateTime dtLastRead)
        {
            ...
            try
            {
                ...
                string archivePath = U.GetSetting2("ArchiveFolder", "");
        ...
        private static bool createReport3(ref List<IpBlock> ipBlocks)
        {
            ...
            try
            {
                ...
                string reportPath = U.GetSetting2("ReportPath", "");
                ...
                string fullPath = reportPath + U.GetSetting2("ReportFilePrefix", "") + DateTime.Now.ToString("yyyyMMdd_HHmmss.fff") + ".txt";
                ...
                .AppendLine("App Report:       " + summary.AppEndDT.ToString(TAG.DTF))
                .AppendLine("")
                ...
                if (summary.AppStartDT != null && summary.AppEndDT != null)
                { 
                    sb.AppendLine(tab + "Since App Start:   " + (summary.AppEndDT - summary.AppStartDT).TotalMilliseconds);
                }
                sb.AppendLine(tab + "Initial Load:      " + elapsedLoadAllData)
                .AppendLine(tab + "Save Data 1:       " + elapsedSaveAllData1)
                .AppendLine(tab + "Read Events:       " + elapsedReadEvents)
                .AppendLine(tab + "Group New Events:  " + elapsedGroupNewEvents)
                .AppendLine(tab + "Select FW Events:  " + elapsedSelectQualifying)
                .AppendLine(tab + "Group FW Events:   " + elapsedGroupQualifyingEvents)
                .AppendLine(tab + "Update Counts:     " + elapsedUpdateCounts)
                .AppendLine(tab + "Firewall Add:      " + elapsedFWAdd)
                .AppendLine(tab + "Firewall Expire:   " + elapsedFWExpire)
                //.AppendLine(tab + "Save Data 2:       " + elapsedSaveAllData2)
                // TODO - the higher-level count still needs to be higher
                // Summary holds an old version report time, where when it is added to the report, it is the time
                // you cannot put a time on what has not happened, or change the overall time of reality
                .AppendLine(tab + "Process Report:    " + summary.ElapsedReport)
                //.AppendLine(tab + "Remove Old Data:   " + elapsedDataAging)
                .AppendLine(tab + "Not Shown:  Remove Old Data")// TODO - Safely migrate summary to the end of processing
                .AppendLine(" -------------------- ")
        ...
        public static bool addFirewallRules2(int minFailuresToBlock, List<IpBlock> ipBlocks)
        {
            ...
            try
            {
                ...
                int msBetweenTestMin = U.GetSetting2("MSBetweenFWTestMin", 250);
                int msBetweenTestMax = U.GetSetting2("MSBetweenFWTestMax", 1200);
                int msBetweenAddMin = U.GetSetting2("MSBetweenFWAddMin", 2000);
                int msBetweenAddMax = U.GetSetting2("MSBetweenFWAddMax", 3800);
                int minutesToReview = U.GetSetting2("FWMinutesToReview", 10080);
                DateTime dtScanStart = DateTime.Now.AddMinutes(-1 * minutesToReview);
                DateTime dtScanEnd = DateTime.Now;

                DateTime activeDate = DateTime.Now;
                int expireAfterDays = U.GetSetting2("FWExpireAfterDays", 30);
                ...
                string port = U.GetSetting2("FWPort", "Any");
                string protocol = U.GetSetting2("FWProtocol", "TCP");

                string prefix = U.GetSetting2("FWPrefix", "ELPRule");
        ...
        public static bool addFirewallRules(int minFailuresToBlock, List<IpBlock> ipBlocks)
        {
            ...
            try
            {
                ...
                int expireAfterMinutes = U.GetSetting2("FWExpireAfterMinutes", 10080);
                ...
                string prefix = U.GetSetting2("FWPrefix", "FWMRule");
