...
namespace FWM_Client_03
{
    ...
    public static class DataMgr
    {
        ...
        public static bool saveAllData(bool includeFWRows)
        {
            ...
            try
            {
                ...
                if (includeFWRows)
                {
                    cntFWRow = FileMgr.writeFWRows(DataMgr.FWRows);
        ...
        */
        ...
        public static FWRow getFWByIpId(long ipId)
        {
            const string location = CLASSNAME + ".getFWById";
            FWRow retVal = null;
            try
            {
                if (ipId <= 0)
                {
                    L.err(location, "Ip id (" + ipId + ") was invalid.");
                    return retVal;
                }

                if (DataMgr.FWRows == null)
                {
                    L.err(location, "Memory was null at request.");
                    return retVal;
                }

                for (int i = 0; i < DataMgr.FWRows.Count; i++)
                {
                    if (ipId != DataMgr.FWRows[i].IpId) 
                        continue;

                    retVal = DataMgr.FWRows[i];
                    break;
                }
            }
            catch (Exception ex)
        ...
        public static List<FWRow> getFWRunningRules()
        {
            const string location = CLASSNAME + ".getFWRunningRules";
            List<FWRow> retVal = new List<FWRow>();
            try
            {
                if (DataMgr.FWRows == null)
                {
                    L.err(location, "Firewall rows not loaded into memory.");
                    return retVal;
                }
                ...
                for (int i = 0; i < DataMgr.FWRows.Count; i++)
                {
                    ...
                    {
                        if (!DataMgr.FWRows[i].Expired)
                        {
                            if (
                                DataMgr.FWRows[i].Deactivated == null || 
                                DataMgr.FWRows[i].Deactivated == c.nDt || 
                                DataMgr.FWRows[i].Deactivated == unset
                            )
                            {
                                retVal.Add(DataMgr.FWRows[i]);
                            }
                        }
                    }
                }
        ...
        public static bool isFWRowForIp(ref List<FWRow> rows, long ipId)
        {
            const string location = CLASSNAME + ".getFWIdxForIp";
            bool retVal = false;
            try
            {
                if (rows == null)
        ...
        public static bool isFWRowForIpBlock(ref List<FWRow> rows, long ipBlockId)
        {
            const string location = CLASSNAME + ".getFWIdxForIpBlock";
            bool retVal = false;
            try
            {
                if (rows == null)
